...
# -*- coding: utf-8 -*-
"""
convert-v9_3dgs_compat.py

v9: 3DGS-compatible layout + enforce PINHOLE/SIMPLE_PINHOLE camera models + pose priors fix.

Your error:
  Check failed: static_cast<int>(pose_prior.coordinate_system) == static_cast<int>(PosePrior::CoordinateSystem::WGS84) (1 vs. 0)

means: in your COLMAP build, PosePrior::CoordinateSystem::WGS84 == 0,
but your script inserted coordinate_system=1.

This script:
- writes pose_priors.coordinate_system = 0 by default for GPS (WGS84).
- keeps Windows-safe execution (handles colmap.bat/cmd) and robust exiftool JSON parsing.
"""
