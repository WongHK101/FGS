...
def _build_argparser() -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        "--building_root",
        default=r"G:\ACMMM26\GeometricRepeatability\DepthReference\Building_full8_reuse_agreement_20260425_v1",
    )
    ap.add_argument(
        "--formal4_root",
        default=r"G:\ACMMM26\GeometricRepeatability\DepthReference\Formal_4scene_8method_9thr_agreement_20260425_v1",
    )
    ap.add_argument(
        "--repo_root",
        default=r"D:\dataset\FGS\FGS-0202v1",
    )
    ap.add_argument(
        "--out_root",
        default=r"G:\ACMMM26\GeometricRepeatability\AssistantPack_DepthReference_5scene8method_20260427_v1",
    )
    ap.add_argument(
        "--dpi",
        type=int,
...
def _write_readme(out_root: Path) -> None:
    readme_text = f"""# Depth-Reference Evaluation Assistant Pack

This package contains the non-large-file deliverables for the 5-scene x 8-method
reference-depth-based geometric evaluation. It is intended for a writing assistant
to inspect the full numeric outputs, threshold curves, protocol files, and code
...
- Building reuse root:
  `G:\\ACMMM26\\GeometricRepeatability\\DepthReference\\Building_full8_reuse_agreement_20260425_v1`
- Formal 4-scene root:
  `G:\\ACMMM26\\GeometricRepeatability\\DepthReference\\Formal_4scene_8method_9thr_agreement_20260425_v1`

## 11. Deliberately Excluded Large Files

The following were intentionally not copied into this assistant pack:
...
If the writing assistant later needs those raw heavy artifacts, they should be
...
def main() -> None:
    ...
    _write_readme(out_root=out_root)
    zip_path = _zip_out_root(out_root=out_root)
    print(f"ASSISTANT_PACK_READY {out_root}")
    print(f"ASSISTANT_PACK_ZIP {zip_path}")
