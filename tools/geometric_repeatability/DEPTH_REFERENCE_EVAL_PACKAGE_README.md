...
& $py "$repo\tools\geometric_repeatability\build_depth_reference.py" `
  --strict_protocol_manifest "G:\path\to\strict_protocol_manifest.json" `
  --out_dir "G:\path\to\DepthReference\SceneName\reference" `
  --colmap_cmd $colmap `
  --resolution_arg 4 `
...
& $py "$repo\tools\geometric_repeatability\export_gaussian_probe_bundle.py" `
  -s "G:\path\to\scene_dataset" `
  -m "G:\path\to\model" `
  -r 4 `
  --iteration 30000 `
  --out_dir "G:\path\to\DepthReference\SceneName\MethodName\probe_bundle" `
  --split_label "MethodName" `
  --scene_name_override "SceneName" `
  --camera_frame_mode probe_manifest_native_align `
  --probe_camera_manifest "G:\path\to\SceneName\reference\probe_camera_manifest.json" `
  --native_cameras_json "G:\path\to\native_cameras.json"
```
...
& $py "$repo\tools\geometric_repeatability\evaluate_depth_reference.py" `
  --reference_manifest "G:\path\to\SceneName\reference\reference_manifest.json" `
  --model_manifest "G:\path\to\SceneName\MethodName\probe_bundle\model_depth_manifest.json" `
  --adapter_manifest "G:\path\to\SceneName\MethodName\probe_bundle\depth_adapter_manifest.json" `
  --out_dir "G:\path\to\SceneName\MethodName\metrics" `
...
& $py "$repo\tools\geometric_repeatability\visualize_strict_probe_method_rgb_depth_comparison.py" `
  --reference_manifest "G:\path\to\SceneName\reference\reference_manifest.json" `
  --out_dir "G:\path\to\SceneName\visual_checks" `
  --scene_name "SceneName" `
  --gt_images_root "G:\path\to\strict_dataset" `
  --gt_images_dir_name "images" `
  --random_n 10 `
  --random_seed 1 `
  --include_unmasked_reference_depth `
  --method "Ours=G:\path\to\Ours\probe_bundle" `
  --method "Baseline=G:\path\to\Baseline\probe_bundle"
```
...
For paper or rebuttal material, include:
