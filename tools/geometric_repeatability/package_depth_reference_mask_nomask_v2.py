...
FORMAL = Path(r"G:\ACMMM26\GeometricRepeatability\DepthReference\Formal_5scene_8method_9thr_d12trim6_20260505_v1")
MESHFIX = Path(r"G:\ACMMM26\GeometricRepeatability\DepthReference\MeshFix_ROICrop_failed3_20260506_v1")
V1 = Path(r"G:\ACMMM26\GeometricRepeatability\AssistantPack_DepthReference_MaskNoMask_5scene8method_20260507_v1")
OUT = Path(r"G:\ACMMM26\GeometricRepeatability\AssistantPack_DepthReference_MaskNoMask_5scene8method_20260507_v2")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build the mask/no-mask depth-reference assistant package")
    parser.add_argument("--formal_root", default=str(FORMAL), help="Root containing method model bundles and Building/Orchard references")
    parser.add_argument("--meshfix_root", default=str(MESHFIX), help="Root containing repaired PVpanel/Road/TransmissionTower references")
    parser.add_argument("--previous_package_root", default=str(V1), help="Previous package root used when copying metrics")
    parser.add_argument("--out", default=str(OUT), help="Output package directory")
...
def configure_from_args(args: argparse.Namespace) -> None:
    ...
    V1 = Path(args.previous_package_root).resolve()
...
def mark_deprecated_targets() -> None:
    if not DEPRECATED_TARGETS:
        return
    marker_text = f"""# Deprecated Depth-Reference Assistant Package
